...
* `cluster_charge_type` - (Optional) Valid values are `Prepaid`, `Postpaid`, Default to `Postpaid`. Currently, the resource can not supports change pay type.
...
* `effective_time` - (Optional) The effective_time of cluster parameters. Valid values are `Auto`, `Immediately`, `MaintainTime`, Default to `Auto`.
