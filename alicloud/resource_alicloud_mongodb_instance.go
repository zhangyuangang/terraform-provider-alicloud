...
func resourceAlicloudMongoDBInstanceUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if d.HasChange("db_instance_storage") ||
		...
		stateConf := BuildStateConf([]string{"Updating", "Modifying"}, []string{"Running"}, d.Timeout(schema.TimeoutUpdate), 3*time.Second, ddsService.RdsMongodbDBInstanceStateRefreshFunc(d.Id(), []string{"Deleting"}))
		if _, err := stateConf.WaitForState(); err != nil {
			return WrapError(err)
		}
