...
func resourceAlicloudDBInstanceUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if !d.IsNewResource() && (d.HasChange("instance_charge_type") || d.HasChange("period")) {
		...
		prePaidRequest.Period = strconv.Itoa(d.Get("period").(int))
		...
		if configPayType == PrePaid {
			...
			stateConf := BuildStateConf([]string{"Updating", "Modifying"}, []string{"Running"}, d.Timeout(schema.TimeoutUpdate), 3*time.Second, rdsService.RdsDBInstanceStateRefreshFunc(d.Id(), []string{"Deleting"}))
			if _, err := stateConf.WaitForState(); err != nil {
				return WrapError(err)
			}
