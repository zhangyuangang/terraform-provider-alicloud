...
func resourceAlicloudKVStoreInstance() *schema.Resource {
	return &schema.Resource{
		...
		Timeouts: &schema.ResourceTimeout{
			Create: schema.DefaultTimeout(DefaultLongTimeout * time.Second),
			Update: schema.DefaultTimeout(DefaultLongTimeout * time.Second),
			Delete: schema.DefaultTimeout(DefaultLongTimeout * time.Second),
		},
...
func resourceAlicloudKVStoreInstanceCreate(d *schema.ResourceData, meta interface{}) error {
	...
	stateConf := BuildStateConf([]string{"Creating"}, []string{"Active", "Running"}, d.Timeout(schema.TimeoutCreate), 3*time.Second, kvstoreService.RdsKvstoreInstanceStateRefreshFunc(d.Id(), []string{"Deleting"}))
...
func resourceAlicloudKVStoreInstanceUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if d.HasChange("security_ips") {
		...
		stateConf := BuildStateConf([]string{"Updating", "Modifying"}, []string{"Active", "Running"}, d.Timeout(schema.TimeoutUpdate), 3*time.Second, kvstoreService.RdsKvstoreInstanceStateRefreshFunc(d.Id(), []string{"Deleting"}))
		if _, err := stateConf.WaitForState(); err != nil {
			return WrapError(err)
		}
...
func resourceAlicloudKVStoreInstanceDelete(d *schema.ResourceData, meta interface{}) error {
	...
	stateConf := BuildStateConf([]string{"Creating", "Active", "Deleting"}, []string{}, d.Timeout(schema.TimeoutDelete), 3*time.Second, kvstoreService.RdsKvstoreInstanceStateRefreshFunc(d.Id(), []string{}))
