...
func (s *PolarDBService) WaitForPolarDBInstance(id string, status Status, timeout int) error {
	...
	for {
		...
		if err != nil {
			if s.NotFoundPolarDBInstance(err) {
				if status == Deleted {
					return nil
				}
			} else {
...
func (s *PolarDBService) NotFoundPolarDBInstance(err error) bool {
	if NotFoundError(err) || IsExceptedErrors(err, []string{InvalidDBClusterIdNotFound, InvalidDBClusterNameNotFound}) {
		return true
	}
	return false
}
...
func (s *PolarDBService) ModifyParameters(d *schema.ResourceData, effectiveTime string, attribute string) error {
...
func (s *PolarDBService) NotFoundCluster(err error) bool {
	if NotFoundError(err) || IsExceptedErrors(err, []string{InvalidDBClusterIdNotFound, InvalidDBClusterNameNotFound}) {
		return true
	}
	return false
