...
func resourceAlicloudPolarDBConnectionRead(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		if polarDBService.NotFoundPolarDBInstance(err) {
			d.SetId("")
			return nil
		}
		return WrapError(err)
...
func resourceAlicloudPolarDBConnectionDelete(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		if polarDBService.NotFoundPolarDBInstance(err) || IsExceptedErrors(err, []string{InvalidCurrentConnectionStringNotFound, AtLeastOneNetTypeExists}) {
			return nil
		}
		return WrapErrorf(err, DefaultErrorMsg, d.Id(), request.GetActionName(), AlibabaCloudSdkGoERROR)
	}
